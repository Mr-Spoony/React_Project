    ...
    render() {
        return (
            <div className=''>
                ...
                {this.props.survivor.data.alive === true && this.props.survivor.data.huntXp === 0 && "New Survivor"}
                {this.props.survivor.data.alive === false && <span className="badge p-2 m-2 bg-danger">Dead</span>}
                {this.props.survivor.data.alive === true && <button onClick={() => this.props.onKill(this.props.survivor.id)} className='btn btn-secondary btn-sm m-2'>Kill</button>}
            </div>
        );  
    }
